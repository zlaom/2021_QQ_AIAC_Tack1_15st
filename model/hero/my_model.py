...
import transformers
from model.hero.layers import (BertPooler, LinearLayer, MLPLayer, BertLMPredictionHead, BertEncoder)
...
class CrossModalTrm(RobertaPreTrainedModel):
    ...
    def forward_vsm(self, batch):
        input_ids = batch['only_input_ids'].cuda()
        title_mask = batch['only_title_mask'].long().cuda()
    ...
    def _compute_embeddings(self, batch):
        input_ids = batch['only_input_ids'].cuda()
        title_mask = batch['only_title_mask'].cuda()
        title_embedding = self.embeddings(input_ids)  # 单纯的投射关系 + pos_embed [+ type_embed]
...
class HierarchicalVlModel(VideoPreTrainedModel):
    def __init__(self, config, vfeat_dim=1536, nce_temp=1.0):
        ...
        self.frame_transform = LinearLayer(vfeat_dim,
                                           config.f_config.hidden_size,
                                           layer_norm=True,
                                           dropout=config.f_config.hidden_dropout_prob,
                                           relu=True)
        self.c_encoder = TemporalTrm(config.c_config)

        # mfm used
        self.feat_regress = FrameFeatureRegression(config.f_config.hidden_size,
                                                   vfeat_dim)  # 768->1536
    ...
    def forward_vsm(self, batch, compute_loss):
        ...
        q2video_scores = self.get_video_level_scores(modularized_query, frame_sequence_outputs,
                                                     batch['frame_mask'].cuda())
    ...
    def get_video_level_scores(self, modularized_query, context_feat1, context_mask):
    ...
    def get_video_level_loss(self, query_context_scores, reduction="mean"):
        ...
        for i, j in zip(range(bsz_v), range(0, bsz_q, num_q_per_v)):
            pos_video_query_scores.append(query_context_scores[j:j + num_q_per_v, i])
            # impossibly large for cosine similarity, the copy is created
            # as modifying the original will cause error
            query_context_scores_masked[j:j + num_q_per_v, i] = 999
        # (Nv, 5)
        ...
        if self.use_all_neg:
            ...
            # (Nq, Nv-1)
            pos_query_neg_context_scores = self.get_all_neg_scores(query_context_scores_masked,
                                                                   sample_min_idx=1)
            # (Nq, Nv-1)
            loss_neg_ctx = self.get_ranking_loss(pos_query_video_scores,
            ...
            # (Nv, Nq-5)
            neg_query_pos_context_scores = self.get_all_neg_scores(video_query_scores_masked,
            ...
            # (Nv, 5, Nq-5)
            loss_neg_q = self.get_ranking_loss(pos_video_query_scores, neg_query_pos_context_scores)
            # (Nq, Nq-5)
            ...
            pos_query_neg_context_scores = self.get_sampled_neg_scores(
                query_context_scores_masked, sample_min_idx=1).unsqueeze(-1)
            # (Nq, 1)
            loss_neg_ctx = self.get_ranking_loss(pos_query_video_scores,
                                                 pos_query_neg_context_scores)
            # (Nv, 1)
            neg_query_pos_context_scores = self.get_sampled_neg_scores(
                video_query_scores_masked, sample_min_idx=num_q_per_v).unsqueeze(-1)
            # (Nv, 5)
            loss_neg_q = self.get_ranking_loss(pos_video_query_scores, neg_query_pos_context_scores)
    ...
    def get_all_neg_scores(self, scores_masked, pos_indices=None, sample_min_idx=1):
        """
        ...
        sorted_scores_masked, sorted_scores_indices = torch.sort(scores_masked,
                                                                 descending=True,
                                                                 dim=1)
    ...
    def get_ranking_loss(self, pos_score, neg_score):
        ...
        if self.ranking_loss_type == "hinge":
            ...
            loss = torch.log1p(torch.exp(neg_score - pos_score))
        else:
    ...
    def get_sampled_neg_scores(self, scores_masked, sample_min_idx=1):
        ...
        sampled_neg_score_indices = sorted_scores_indices[pos_indices,
                                                          torch.randint(
                                                              sample_min_idx,
                                                              sample_max_idx,
                                                              size=(num_neg, )).to(
                                                                  scores_masked.device)]  # (N, )
        sampled_neg_scores = scores_masked[pos_indices, sampled_neg_score_indices]  # (N, )
