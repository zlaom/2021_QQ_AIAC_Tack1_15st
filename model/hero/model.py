...
class VideoModelConfig(object):
    def __init__(self, config_json_file):
        ...
        if "sep_config" in config:
            self.sep_config = RobertaModelConfig.from_dict(config["sep_config"])
        else:
            self.sep_config = None

        if "mix_config" in config:
            self.mix_config = RobertaModelConfig.from_dict(config["mix_config"])
        else:
...
class TagLinear(nn.Module):
    def __init__(self, hidden_size, inter_size = 2048, class_size = 10000):
        super().__init__()
        self.net = nn.Sequential(nn.Linear(hidden_size, inter_size),
                                 GELU(),
                                 FusedLayerNorm(inter_size, eps=1e-5),
                                 nn.Linear(inter_size, class_size))
