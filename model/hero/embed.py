...
class SubEmbeddings(nn.Module):
    ...
    def forward(self, input_ids=None, position_ids=None,
        ...
        #     token_type_embeddings = self.token_type_embeddings(torch.zeros(1, 1, dtype=torch.long, device=device))
        # else:
...
class ImageEmbeddings(nn.Module):
    def __init__(self, config, img_dim, max_img_seq_len):
        ...
        self.position_embeddings = nn.Embedding(max_img_seq_len, config.hidden_size)
        self.mask_embedding = nn.Embedding(2, img_dim, padding_idx=0)

        self.LayerNorm = FusedLayerNorm(config.hidden_size, eps=1e-5)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, img_feat, type_embeddings=None, img_pos_ids=None, img_masks=None):
        if img_pos_ids is None:
            img_pos_ids = self.create_position_ids_from_inputs_embeds(img_feat)

        if img_masks is not None: # 这是mfm用的
